...
public class EditorAnimaClip : ScriptableObject
{
    public int ID;
...
public class EditorAminaClipLoadWindow : EditorWindow
{
    ...
    public void Init(AminaEditor _aminaEditor, OriginAminType _protype)
    {
        ...
        scroll = 0;
        allClips = new List<EditorAnimaClip>();
        ...
        {
            EditorAnimaClip _new = AssetDatabase.LoadAssetAtPath<EditorAnimaClip>(_all[i]);
    ...
    private void Rename()
    {
        ...
        {
            EditorAnimaClip _new = AssetDatabase.LoadAssetAtPath<EditorAnimaClip>(_all[i]);
            if (_new==null) { Debug.Log("meiyou new"); }
