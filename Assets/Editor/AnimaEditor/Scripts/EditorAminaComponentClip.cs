...
[Serializable]
public class FrameOutputData_Atlas
{
    [SerializeField]
    public int Index { get { return index; } }
    [SerializeField]
    private int index;
    public List<OutType_Atlas> Out;

    public FrameOutputData_Atlas() { Out = new List<OutType_Atlas>(); }
    public FrameOutputData_Atlas(FrameOutputData_Atlas other)
    {
        index = other.Index;
        Out = new List<OutType_Atlas>(other.Out);
    }

    public FrameOutputData_Atlas(int _index)
    {
        index = _index;
        Out = new List<OutType_Atlas>();
    }
    ...
    public AnimaClipEvent ToAnimaClipEvent()
    {
        if (Out.Count == 0)
        {
        ...
        AnimaClipEvent result = new AnimaClipEvent();
        result.index = index;
        result.eventType = new int[Out.Count];
        for(int i = 0; i < Out.Count; i++)
        {
            result.eventType[i] = (int)Out[i];
        }
